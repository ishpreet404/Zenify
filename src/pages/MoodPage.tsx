...
const MoodPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const validEntries = loadedEntries.filter(entry => {
      const isValid = !isNaN(new Date(entry.date).getTime());
      if (!isValid) {
  ...
  // Check if the user has already tracked mood today
  ...
  const addMoodEntry = (mood: Mood, note: string): MoodEntry => {
    ...
    storage.saveMoodEntries(updatedEntries); // Save to storage
    return newEntry;
  };

  // Handle tracking mood
  const handleTrackMood = () => {
    addMoodEntry(currentMood, moodNote);
    setMoodNote('');
  };
  ...
  return (
    <div>
      ...
      >
        ...
        <MoodChart
          moodEntries={moodEntries.filter(entry => !isNaN(new Date(entry.date).getTime()))}
          days={timeframe}
        />
      </motion.div>
